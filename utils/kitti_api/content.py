...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dataset', '-d', type=str, required=True, help='Dataset to calculate content. No Default',
    )
    parser.add_argument(
        '--config',
        '-c',
    ...
    for seq in sequences:
        ...
        scan_names = [
            os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(scan_paths)) for f in fn
        ]
        scan_names.sort()
        ...
        # populate the pointclouds
        label_names = [
            os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(label_paths)) for f in fn
        ]
        label_names.sort()
        ...
        seq_accum = collections.OrderedDict(sorted(seq_accum.items(), key=lambda t: t[0]))
