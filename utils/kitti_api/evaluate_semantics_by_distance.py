...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        required=False,
        default="numpy",
        help='Backend for evaluation. One of ' + str(backends) + ' Defaults to %(default)s',
    )
    parser.add_argument(
        '--datacfg',
        '-dc',
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help='Exports "segmentation_scores_distance.txt" for codalab' 'Defaults to %(default)s',
    )
    ...
    for sequence in test_sequences:
        ...
        label_paths = os.path.join(FLAGS.dataset, "sequences", str(sequence), "velodyne")
    ...
    for idx in range(len(DISTANCES)):
        ...
        sys.stdout.write(
            'range {lrange}m to {hrange}m,'.format(lrange=DISTANCES[idx][0], hrange=DISTANCES[idx][1])
        )
