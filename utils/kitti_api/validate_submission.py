    ...
    try:
        ...
        with zipfile.ZipFile(FLAGS.zipfile) as zipfile:
            if FLAGS.task == "segmentation" or FLAGS.task == "panoptic":
                ...
                directories = [
                    folder.filename for folder in zipfile.infolist() if folder.filename.endswith("/")
                ]
                ...
                for sequence in range(11, 22):
                    ...
                    if sequence_directory not in directories:
                        raise ValidationException(
                            'Directory "{}" missing inside zip file.'.format(sequence_directory)
                        )
                    predictions_directory = sequence_directory + "predictions/"
                    if predictions_directory not in directories:
                        raise ValidationException(
                            'Directory "{}" missing inside zip file.'.format(predictions_directory)
                        )
                ...
                prediction_files = {
                    info.filename: info for info in zipfile.infolist() if not info.filename.endswith("/")
                }
                ...
                for sequence in range(11, 22):
                    ...
                    velodyne_files = sorted(
                        [os.path.join(velodyne_directory, file) for file in os.listdir(velodyne_directory)]
                    )
                    label_files = sorted(
                        [
                            os.path.join(
                                sequence_directory, "predictions", os.path.splitext(filename)[0] + ".label"
                            )
                ...
                if len(necessary_files) != len(prediction_files.keys()):
                    ...
                    if len(filelist) > 10:
                        ell = ", ..."
                    raise ValidationException(
                        "Zip contains unneeded predictions, e.g., {}".format(",".join(filelist[:10]) + ell)
                    )
