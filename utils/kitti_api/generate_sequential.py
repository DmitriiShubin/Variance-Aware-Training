...
if __name__ == '__main__':
    ...
    sequence_folders = [
        f for f in sorted(os.listdir(sequences_dir)) if os.path.isdir(os.path.join(sequences_dir, f))
    ]
    ...
    for folder in sequence_folders:
        ...
        if os.path.exists(output_folder) or os.path.exists(velodyne_folder) or os.path.exists(labels_folder):
        ...
        scan_files = [
            f for f in sorted(os.listdir(os.path.join(input_folder, "velodyne"))) if f.endswith(".bin")
        ]
        ...
        for i, f in enumerate(scan_files):
            ...
            history.appendleft(
                {"points": points, "labels": labels, "remissions": remissions, "pose": pose.copy()}
            )
