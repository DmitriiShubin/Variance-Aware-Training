...
if __name__ == '__main__':
    ...
    for sequence in test_sequences:
        ...
        seq_label_names = sorted(
            [os.path.join(label_paths, fn) for fn in os.listdir(label_paths) if fn.endswith(".label")]
        )
    ...
    for sequence in test_sequences:
        ...
        seq_pred_names = sorted(
            [os.path.join(pred_paths, fn) for fn in os.listdir(pred_paths) if fn.endswith(".label")]
        )
    ...
    PQ_dagger = np.mean(
        [float(output_dict[c]["PQ"]) for c in things] + [float(output_dict[c]["IoU"]) for c in stuff]
    )
