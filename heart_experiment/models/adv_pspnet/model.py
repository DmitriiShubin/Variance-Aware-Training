...
# model
from models.adv_pspnet.structure import PSPNet
...
class Model:
    ...
    def __init__(self, n_channels, hparams, gpu, inference=False):
        ...
        if inference:
            ...
            self.model = PSPNet(hparams=self.hparams, n_channels=n_channels, n_classes=2).to(self.device)
        else:
            if torch.cuda.device_count() > 1:
                if len(gpu) > 0:
                    print("Number of GPUs will be used: ", len(gpu))
                    self.device = torch.device(f"cuda:{gpu[0]}" if torch.cuda.is_available() else "cpu")
                    self.model = PSPNet(hparams=self.hparams, n_channels=n_channels, n_classes=2).to(
                        self.device
                    )
