...
def conv3D_output_shape_f(i, input_shape, kernel_size, dilation, padding, stride):
    ...
    return (
        math.floor(
            (input_shape[i] - kernel_size[i] - (dilation[i] - 1) * (kernel_size[i] - 1) + 2 * padding[i])
            / stride[i]
        )
        + 1
    )
...
def acs_conv_f(
    x, weight, bias, kernel_size, dilation, padding, stride, groups, out_channels, acs_kernel_split
):
