...
class SoftACSConv(_ACSConv):
        ...
        return (
            math.floor(
                (
                    input_shape[i]
                    - self.kernel_size[i]
                    - (self.dilation[i] - 1) * (self.kernel_size[i] - 1)
                    + 2 * self.padding[i]
                )
                / self.stride[i]
            )
    ...
    def forward(self, x):
        ...
        conv3D_output_shape = (
            self.conv3D_output_shape_f(0, input_shape),
        ...
        f_a = F.conv3d(
            ...
            else F.pad(x, (0, 0, 0, 0, self.padding[0], self.padding[0]), 'constant', 0)[
                :,
                :,
        ...
        f_c = F.conv3d(
            ...
            else F.pad(x, (0, 0, self.padding[1], self.padding[1]), 'constant', 0)[
                :,
                :,
                :,
                self.kernel_size[1] // 2 : self.kernel_size[1] // 2
                + self.stride[1] * (conv3D_output_shape[1] - 1)
                + 1,
                :,
            ],
            weight=self.weight.unsqueeze(3),
            bias=None,
            stride=self.stride,
        ...
        f_s = F.conv3d(
            ...
            else F.pad(x, (self.padding[2], self.padding[2]), 'constant', 0)[
                :,
                :,
                :,
                :,
                self.kernel_size[2] // 2 : self.kernel_size[2] // 2
                + self.stride[2] * (conv3D_output_shape[2] - 1)
                + 1,
            ],
            weight=self.weight.unsqueeze(4),
            bias=None,
            stride=self.stride,
        ...
        if self.mean:
            ...
            f = (
                f_a * soft_w[:, 0].view(1, self.out_channels, 1, 1, 1)
