    ...
    def convert_module(self, module):
        ...
        for child_name, child in module.named_children():
            if isinstance(child, nn.Conv2d):
                ...
                'pool' in child.__class__.__name__.lower() or 'norm' in child.__class__.__name__.lower()
            ):
                if hasattr(nn, child.__class__.__name__.replace('2d', '3d')):
                    ...
                    raise Exception(
                        'No corresponding module in 3D for 2d module {}'.format(child.__class__.__name__)
                    )
