...
class Model:
    ...
    def fit(self, train, valid):
        ...
        for epoch in range(self.hparams['n_epochs']):
            ...
            # calc train metrics
            metric_train = self.metric.compute(self.hparams['dataset'])
            ...
            # calc val metrics
            metric_val = self.metric.compute(self.hparams['dataset'])
    ...
    def predict(self, X_test):
        ...
        fold_score = self.metric.compute(self.hparams['dataset'])
    ...
    def __setup_model(self, inference, gpu):
        ...
        if inference or gpu is None:
            ...
            self.model = EfficientNet(hparams=self.hparams['model']).to(self.device)
        else:
