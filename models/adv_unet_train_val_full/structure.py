...
class UNet(nn.Module):
    def __init__(self, hparams, bilinear=False):
        ...
        for i in range(5 + 1):
            if i < 5:
                n_filt += 2*self.hparams['n_filters_input'] * (2 ** (i))
            else:
    ...
    def forward(self, x1, x2=None, train=False):
        ...
        if train:
            ...
            out,encoder_x, decoder_x = self.predictive_network(x1)
            # additional head (adversarial)
            out_s = self.adversarial_network(encoder_x, decoder_x, x2)
            return out, out_s
        else:
            # main head (predictive)
            out, _,_ = self.predictive_network(x1)
            return out
    ...
    def adversarial_network(self, encoder_x,decoder_x, x_s):
        encoder_xs = self.encoder(x_s)
        _,decoder_xs = self.decoder(*encoder_xs)

        x1_s = self.rever1_1(encoder_xs[0]).mean(dim=2).mean(dim=2)
        x2_s = self.rever1_2(encoder_xs[1]).mean(dim=2).mean(dim=2)
        x3_s = self.rever1_3(encoder_xs[2]).mean(dim=2).mean(dim=2)
        x4_s = self.rever1_4(encoder_xs[3]).mean(dim=2).mean(dim=2)
        x5_s = self.rever1_5(encoder_xs[4]).mean(dim=2).mean(dim=2)
        x6_s = self.rever1_6(encoder_xs[5]).mean(dim=2).mean(dim=2)
        x7_s = self.rever1_7(decoder_xs[0]).mean(dim=2).mean(dim=2)
        x8_s = self.rever1_8(decoder_xs[1]).mean(dim=2).mean(dim=2)
        x9_s = self.rever1_9(decoder_xs[2]).mean(dim=2).mean(dim=2)
        x10_s = self.rever1_10(decoder_xs[3]).mean(dim=2).mean(dim=2)
        x11_s = self.rever1_11(decoder_xs[4]).mean(dim=2).mean(dim=2)


        x1_p = self.rever2_1(encoder_x[0]).mean(dim=2).mean(dim=2)
        x2_p = self.rever2_2(encoder_x[1]).mean(dim=2).mean(dim=2)
        x3_p = self.rever2_3(encoder_x[2]).mean(dim=2).mean(dim=2)
        x4_p = self.rever2_4(encoder_x[3]).mean(dim=2).mean(dim=2)
        x5_p = self.rever2_5(encoder_x[4]).mean(dim=2).mean(dim=2)
        x6_p = self.rever1_6(encoder_x[5]).mean(dim=2).mean(dim=2)
        x7_p = self.rever2_1(decoder_x[0]).mean(dim=2).mean(dim=2)
        x8_p = self.rever2_2(decoder_x[1]).mean(dim=2).mean(dim=2)
        x9_p = self.rever2_3(decoder_x[2]).mean(dim=2).mean(dim=2)
        x10_p = self.rever2_4(decoder_x[3]).mean(dim=2).mean(dim=2)
        x11_p = self.rever2_5(decoder_x[4]).mean(dim=2).mean(dim=2)


        x = torch.cat([x1_s, x2_s, x3_s, x4_s, x5_s, x6_s, x7_s, x8_s, x9_s, x10_s, x11_s,
                       x1_p, x2_p, x3_p, x4_p, x5_p,x6_p, x7_p, x8_p, x9_p, x10_p, x11_p], dim=1)
    ...
    def predictive_network(self, x):
        ...
        x,decoder_outputs = self.decoder(x1, x2, x3, x4, x5,x6)
        logits = self.outc(x)
        logits = torch.nn.functional.softmax(logits, dim=1)
        return logits, [x1, x2, x3, x4, x5,x6],decoder_outputs
