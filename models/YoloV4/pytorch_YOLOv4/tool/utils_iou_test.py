...
def bboxes_iou_test(bboxes_a, bboxes_b, fmt='voc', iou_type='iou'):
    ...
    if fmt.lower() == 'voc':  # xmin, ymin, xmax, ymax
        ...
        # bottom right
        br_intersect = torch.min(
            (bboxes_a[:, np.newaxis, :2] + bboxes_a[:, np.newaxis, 2:]), (bboxes_b[:, :2] + bboxes_b[:, 2:])
        )
        bb_a = bboxes_a[:, 2:]
        bb_b = bboxes_b[:, 2:]
    ...
    en = (
        (tl_intersect < br_intersect).type(tl_intersect.type()).prod(dim=2)
    )  # shape `(N,K,2)` ---> shape `(N,K)`
    ...
    if N == K == 1:
        ...
        product_of_lengths = torch.norm(bb_a, p='fro', dim=1)[:, np.newaxis] * torch.norm(
            bb_b, p='fro', dim=1
        )
        print(f"inner product of bb_a and bb_b is {inner_product}")
        print(f"product of lengths of bb_a and bb_b is {product_of_lengths}")
        print(
            f"inner product divided by product of lengths equals {_true_divide(inner_product, product_of_lengths)}"
        )
        ...
        bc = ED(
            {
                "xmin": tl_union.numpy().astype(int)[0][0][0],
                "ymin": tl_union.numpy().astype(int)[0][0][1],
                "xmax": br_union.numpy().astype(int)[0][0][0],
                "ymax": br_union.numpy().astype(int)[0][0][1],
            }
        )
        ...
        bc.xmin, bc.ymin, bc.xmax, bc.ymax = (
            bc.xmin - adjust_x,
            bc.ymin - adjust_y,
            bc.xmax - adjust_x,
            bc.ymax - adjust_y,
        ...
        if fmt.lower() == 'voc':  # xmin, ymin, xmax, ymax
            ...
            ba = ED(
                {
                    "xmin": ba[0] - ba[2] // 2 - adjust_x,
                    "ymin": ba[1] - ba[3] // 2 - adjust_y,
                    "xmax": ba[0] + ba[2] // 2 - adjust_x,
                    "ymax": ba[1] + ba[3] // 2 - adjust_y,
                }
            )
            bb = ED(
                {
                    "xmin": bb[0] - bb[2] // 2 - adjust_x,
                    "ymin": bb[1] - bb[3] // 2 - adjust_y,
                    "xmax": bb[0] + bb[2] // 2 - adjust_x,
                    "ymax": bb[1] + bb[3] // 2 - adjust_y,
                }
            )
        elif fmt.lower() == 'coco':  # xmin, ymin, w, h
            ba = ED(
        ...
        plane = np.full(
            shape=(int(1.5 * (bc.ymax - bc.ymin)), int(1.5 * (bc.xmax - bc.xmin)), 3),
            fill_value=255,
            dtype=np.uint8,
        )
        ...
        if cv2:
            ...
            cv2.rectangle(
                img_with_boxes,
                (max(0, bc.ymin - 1), max(0, bc.ymin - 1)),
                (bc.xmax, bc.ymax),
                (255, 0, 0),
                line_size,
            )
            ...
            drawer.rectangle(
                [(max(0, bc.xmin - 1), max(0, bc.ymin - 1)), (bc.xmax + 1, bc.ymax + 1)],
                outline='red',
                width=line_size,
            )
