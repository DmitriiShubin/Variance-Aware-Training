...
def evaluate_on_coco(cfg, resFile):
    ...
    sorted_annotations = list(
        sorted(unsorted_annotations, key=lambda single_annotation: single_annotation["image_id"])
    )
    ...
    for i, image_id in enumerate(reshaped_annotations):
        ...
        gt_annotation_image_raw = list(
            filter(lambda image_json: image_json['id'] == image_id, gt_annotation_raw_images)
        )
        gt_annotation_labels_raw = list(
            filter(lambda label_json: label_json['image_id'] == image_id, gt_annotation_raw_labels)
        )
...
def get_args(**kwargs):
    ...
    parser = argparse.ArgumentParser(
        description='Test model on test dataset', formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='model config file to load',
        dest='model_config',
    )
...
if __name__ == "__main__":
    ...
    with open(annotations_file_path) as annotations_file:
        try:
            ...
            exit()
    test(
        model=model, annotations=annotations, cfg=cfg,
    )
